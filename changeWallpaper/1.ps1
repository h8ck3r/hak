...
$Test = $drive + "payloads\switch1\background.jpg"

#Sets variable to test the path to file (background.bmp)
$Switch1 = Test-Path $Test

#Finds what switch bashbunny is set too
if ($Switch1 -eq "True") {$Path = $drive + "payloads\switch1\background.jpg"}
else {$Path = $drive + "payloads\switch2\background.jpg"}
